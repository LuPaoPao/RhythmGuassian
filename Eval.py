...
# Compute per-clip average HR and aggregate into per-video metrics.
# Notes: (1) ~5 files have misaligned timing and need to be deleted;
#        (2) some clips fail face landmark detection.
...
if __name__ == '__main__':
    ...
    frames_num = 256
    pr_path = r'./Result/rPPGNet_VIPLSpatial0.5Temporal0.1HR_pr.mat'
    gt_path = r'./Result/rPPGNet_VIPLSpatial0.5Temporal0.1HR_rel.mat'
    Idex_files = os.environ.get(
        'RPPG_INDEX_DIR',
        os.path.join(os.environ.get('RPPG_DATA_ROOT', './Data/STMap/'), 'STMap_Index/VIPL'),
    )
    ...
    for HR_index in range(pr.size-1):
        ...
        if lastPath != nowPath:
            if pr_temp is None:
                ...
                pr_temp = []
                gt_temp = []
            if math.isnan(np.nanmean(pr_temp)):
                print(nowPath)
                print(lastPath)
                pr_temp = []
                gt_temp = []
