...
#
TARGET_DOMAIN = {
    'VIPL': ['V4V', 'PURE', 'BUAA', 'UBFC', 'UCLA-rPPG'], # , 'MR-NIRP' ,'VV100', 'UCLA-rPPG', 'Phys', "MMPD"
    'V4V': ['VIPL', 'PURE', 'BUAA', 'UBFC', 'UCLA-rPPG']
}
...
def main():
    ...
    # Set RPPG_DATA_ROOT to the STMap root directory
    ...
    log = Logger()
    log_path = os.path.join('./Result_log', f"UCLA{rPPGNet_name}_{para_name}log.txt")
    ...
    for iter_num in range(max_iter + 1):
        ...
        bvp_targets = torch.cat(bvp_targets, dim=0).unsqueeze(1)
        hr_rels = torch.cat(hr_rels, dim=0)

        inputs_aug = torch.cat(inputs_aug, dim=0)
        bvp_targets_aug = torch.cat(bvp_targets_aug, dim=0)
        hr_rels_aug = torch.cat(hr_rels_aug, dim=0)

        optimizer.zero_grad()

        # Forward pass
        bvp_pre, HR_pr, av = BaseNet(inputs)
        bvp_pre_aug, HR_pr_aug, av_aug = BaseNet(inputs_aug)
        ...
        # Additional losses
        av_combined = torch.cat((av, av_aug), dim=0)
        ...
        if iter_num % 200 == 0:
            ...
            log_message = (
                f'Train Iter: {iter_num} | loss: {loss.item():.4f} | Time: {time_to_str(elapsed, "min")}'
            )
        ...
        if iter_num % 2000 == 0 and iter_num>1000:
            ...
            torch.save(BaseNet, model_save_path)
